...
function generateSmbConf(config, shares) {
  const lines = [
    '# Generated by NimOS — do not edit manually',
  ...
  // Generate share sections from NimOS shares
  for (const share of shares) {
...
function handlePortal(url, method, body, req) {
  ...
  if (url === '/api/portal/config' && method === 'POST') {
    ...
    if (hp && hp !== PORT) {
      run(`sudo ufw allow ${hp}/tcp comment 'NimOS Web UI' 2>/dev/null`);
    }
...
function handleDns(url, method, body, req) {
  ...
  if (url === '/api/dns/config' && method === 'POST') {
    ...
    const lines = ['# Generated by NimOS'];
...
function handleRemoteAccess(url, method, body, req) {
  ...
  if (url === '/api/remote-access/enable-https' && method === 'POST') {
    ...
    if (enabled) {
      ...
      const nginxConf = [
        `# NimOS HTTPS — managed by Remote Access panel`,
        `server {`,
      ...
      try {
        ...
        run(`sudo ufw allow ${httpsPort}/tcp comment "NimOS HTTPS" 2>/dev/null`);
...
`;
  ...
  const conf = `# NimOS WebDAV — managed by WebDAV panel
server {
...
function handleWebdav(url, method, body, req) {
  ...
  if (url === '/api/webdav/start' && method === 'POST') {
    ...
    try {
      ...
      const config = getWebdavConfig();
      run(`sudo ufw allow ${config.httpPort || 5005}/tcp comment 'NimOS WebDAV' 2>/dev/null`);
      return { ok: true };
    } catch (err) {
...
function handleSmb(url, method, body, req) {
  ...
  // POST /api/smb/sync-users — sync all NimOS users to Linux/Samba (admin only)
